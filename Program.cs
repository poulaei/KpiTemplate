...
public static class ExpressionEvaluator
{
    public static double Eval(string expression, Dictionary<string, List<double>> data, Dictionary<string, double> computed)
    {
        ...
        if (TryAggregate(expr, "sum", data, out var sumValue))
            return sumValue;

        if (TryAggregate(expr, "avg", data, out var avgValue))
            return avgValue;
    ...
    private static double Resolve(string token, Dictionary<string, List<double>> data, Dictionary<string, double> computed)
    {
        ...
        if (TryAggregate(token, "sum", data, out var sumValue))
            return sumValue;

        if (TryAggregate(token, "avg", data, out var avgValue))
            return avgValue;

        throw new InvalidOperationException($"Cannot resolve token '{token}'.");
    }

    private static bool TryAggregate(string expr, string functionName, Dictionary<string, List<double>> data, out double result)
    {
        result = 0;
        var prefix = functionName + "(";

        if (!expr.StartsWith(prefix, StringComparison.OrdinalIgnoreCase) || !expr.EndsWith(")", StringComparison.Ordinal))
            return false;

        var inner = expr[prefix.Length..^1].Trim();

        if (string.IsNullOrWhiteSpace(inner) || inner.Contains('(') || inner.Contains(')') || inner.Contains(' ') || inner.Contains('/') || inner.Contains('*') || inner.Contains('+') || inner.Contains('-'))
            return false;

        if (!data.TryGetValue(inner, out var arr))
            throw new InvalidOperationException($"Data array '{inner}' not found.");

        if (functionName.Equals("avg", StringComparison.OrdinalIgnoreCase) && arr.Count == 0)
            throw new InvalidOperationException($"Data array '{inner}' not found or empty.");

        result = functionName.Equals("sum", StringComparison.OrdinalIgnoreCase) ? arr.Sum() : arr.Average();
        return true;
    }
}
